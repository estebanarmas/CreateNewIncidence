  ...
  return declare([], {
    ...
    _createForm: function (fields) {
      ...
      if (this._formLayer) {
		...
		console.log(this._formLayer.url);
      }
	...
	_loadNumberFeature: function () {
		...
		query1.orderByFields = ["created_date DESC"];

		var featureLayer = new FeatureLayer(this._formLayer.url,{
		...
		featureLayer.selectFeatures(query1,  FeatureLayer.SELECTION_NEW,  function(features) {
        //TODO: Comprobar si el feature está en un estado válido para este usuario
        if (features.length > 0) {
			console.log(features[0].attributes['OBJECTID']);
      var numeroFinal = parseInt(features[0].attributes['OBJECTID'])+1;
			$("#CODIGO").val("MANSEV"+numeroFinal);
        }else{
